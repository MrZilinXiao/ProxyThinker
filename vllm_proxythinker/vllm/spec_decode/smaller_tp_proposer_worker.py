...
class SmallerTpProposerWorker(ProposerWorkerBase):
    ...
    def maybe_wrap_worker(cls, worker, draft_tensor_parallel_size: int,
                          target_tensor_parallel_size: int, is_positive: bool):
        ...
        if is_positive:
            ...
            # negative rank=4 needs to be set to is_driver_worker
            draft_ranks = list(range(draft_tensor_parallel_size,
                                     target_tensor_parallel_size))
            # if get_tp_group().rank_in_group == draft_ranks[0]:
            #     logger.info(
        ...
        logger.info("SmallerTpProposerWorker: Wrapping {%s} in {%s}", type(worker), cls)
    ...
    def barrier(self) -> None:
        """Barrier for all ranks in the tp group.
        """
        if self._is_dummy:
            return
    ...
    def init_device(self) -> None:
        ...
        print(f"SmallerTpProposerWorker: init_device on {get_tp_group().rank} for type {type(self._worker)} is_dummy: {self._is_dummy} draft_ranks: {self._draft_ranks}")
    ...
    def load_model(self) -> None:
        if self._is_dummy:
            return
        # print(
        #     f"SmallerTpProposerWorker: loading model on {get_tp_group().rank} "
        #     f"with tp group {self._tp_group}")
        with self._patch_tensor_parallel_group():
            # print(f"Try to get get_tensor_model_parallel_world_size: "
            #       f"{get_tp_group().world_size} inside patch_tensor_parallel_group")
    ...
    def prepare_input(
        ...
        # print(f"SmallerTpProposerWorker good to see you here! prepare_input on {get_tp_group().rank} for type {type(self._worker)}")
        if self._is_dummy:
            return

        with self._patch_tensor_parallel_group():
            return self._worker.prepare_input(execute_model_req)
