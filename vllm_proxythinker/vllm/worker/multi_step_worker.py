...
class MultiStepWorker(Worker):
    ...
    def prepare_input(
        ...
        """
        # print(f"MultiStepWorker rank {get_tp_group().rank} is_driver_worker: {self.is_driver_worker} prepare_input")
        if self.is_driver_worker:
            # print(f"MultiStepWorker rank {get_tp_group().rank}: {execute_model_req}")
            ...
            virtual_engine = execute_model_req.virtual_engine
            # print(f"MultiStepWorker is_driver_worker: {self.rank}: broadcasting data...")  # this is only on rank 0!
        ...
        # if TP workers
            ...
            # print(f"MultiStepWorker is not driver worker: {self.rank}.")
            broadcast_data = self._get_worker_input_from_broadcast()
            # print(f"MultiStepWorker got worker_input_from_broadcast: {self.rank}.")
